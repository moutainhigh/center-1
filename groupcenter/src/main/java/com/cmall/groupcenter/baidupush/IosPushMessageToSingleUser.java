 ...
 *推送广播消息、通知
 * */
public class IosPushMessageToSingleUser {

	/**
	 * 发送通知信息
	 * @param {title:xxx(必填),content:xxx(必填), params:xxx(必填),apiKey:xxx(必填),secretKey:xxx(必填),deployStatus:xxx(1: Developer 2: Production必填),userId:xxx(可填),channelId:xxx(可填)}
	 * */
    public MWebResult sendNotifyMsg(MDataMap map) {
        ...
        try {
            ...
            request.setDeployStatus(Integer.parseInt(map.get("deployStatus"))); // DeployStatus => 1: Developer 2: Production
            request.setMessage("{\"aps\":{\"alert\":\""+map.get("content")+"\"},"+map.get("params")+"}");

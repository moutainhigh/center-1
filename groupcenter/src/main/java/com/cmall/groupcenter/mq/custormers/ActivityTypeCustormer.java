 ...
 */
@Deprecated
public class ActivityTypeCustormer implements ChannelAwareMessageListener{

	@Override
	public void onMessage(Message message, Channel channel) {
		
		try {
			List<ActivityTypeListenModel> modelList = JSONArray.parseArray(new String(message.getBody()), ActivityTypeListenModel.class);
			...
			if(null != modelList && modelList.size() > 0) {
				...
				mDataMap.put("queue_code", QueueName.activityTypeQueueName);

...
public class FuncCaseEdit  extends RootFunc{
...
//			mResult.setResultMessage("合作案例url必须以http://开头");
			...
			if(casename.length()<=30){
				...
				Object value = DbUp.upTable("gc_cooperative_case").dataGet("uid", "uid!='"+uid+"' and case_name='"+casename+"'", mWhereMap);
...
//							mResult.setResultMessage("合作案例图标必须是图片格式");
							...
							mWhereMap.put("update_member_code", UserCode);
							...
							DbUp.upTable("gc_cooperative_case").dataUpdate(mWhereMap,"","uid");

 ...
 * @version 1.0
 **/
public class EditWgsTask extends RootFunc{
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		if (mResult.upFlagTrue()) {
			...
			//删除原有的任务步骤及奖励信息
			DbUp.upTable("nc_wgs_task_detail").delete("tid",mAddMaps.get("tid"));
			DbUp.upTable("nc_wgs_task_reward").delete("tid",mAddMaps.get("tid"));
			
			//任务表插入参数
			mAddMaps.put("update_time",create_time);
			mAddMaps.put("creator",publisher);/*获取当前登录人*/
			mAddMaps.put("is_delete","449747110001");//默认未删除
			mAddMaps.put("reward_msg",saveItemsAndRewards(mAddMaps.get("tid"), task));//拼装奖励信息

 ...
 */
public class FuncTraderRebateShutNew extends RootFunc{
...
//        System.out.println("test");
        ...
        Map<String, Object> map =  DbUp.upTable("gc_app_rebate_scale_money").dataSqlOne("select * from gc_app_rebate_scale_money where uid=:uid", mAddMaps);
        ...
        if("1".equals(flag_enable) && StringUtils.isNotEmpty(uid)){//为有效状态，则将数据更改为失效状态。
            ...
            DbUp.upTable("gc_app_rebate_scale_money").dataUpdate(updateMap,"flag_enable","uid");
        }else if ("0".equals(flag_enable) && StringUtils.isNotEmpty(uid)){//如果已经为无效状态，则此次操作是为了删除数据。
            MDataMap updateMap = new MDataMap();
            updateMap.put("delete_flag","0");
            updateMap.put("uid",uid);
            DbUp.upTable("gc_app_rebate_scale_money").dataUpdate(updateMap, "delete_flag", "uid");
        }else {

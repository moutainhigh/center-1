 ...
 * type:449746990025
 * @author 周恩至
 * 此处的returnCode均为APP的returnCode，写入定时任务时，需要特别注意，如果商品为TV品，需要去LD查询是否有数据
 */
public class JobForAgentApplyReturn extends RootJobForExec {
	...
	private MWebResult operateTV(Map<String, Object> returnInfoTV,String returnCode) {
		...
		MDataMap orderInfo = DbUp.upTable("oc_orderinfo").one("out_order_code",ld_order_code, "small_seller_code", "SI2003");
	...
	private void caculate(String memberCode,BigDecimal rate,int type,String orderCode) {
		...
		profitDetial.put("money_type", "4497484600070001");// 冻结收益增加
		...
		profitDetial.put("uid", UUID.randomUUID().toString().replace("-", "").trim());
		profitDetial.put("profit", "-"+rate.toString());
		profitDetial.put("money_type", "4497484600070002");// 提现收益减少
		profitDetial.put("create_time", DateUtil.getSysDateTimeString());
		DbUp.upTable("fh_agent_profit_detail").dataInsert(profitDetial);
	}
	...
	static {
		config.setExecType("449746990025");
	}

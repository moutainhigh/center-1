 ...
 *
 */
public class JobForLdAfterSaleReturn extends RootJob {
	@Transactional
	@Override
	public void doExecute(JobExecutionContext context) {
		 ...
		 */
		String sql = "SELECT * FROM ordercenter.oc_after_sale_ld WHERE after_sale_status = '01' and if_post = '2' and after_sale_type = '1' LIMIT 0,2000";
		List<Map<String, Object>> list = DbUp.upTable("oc_after_sale_ld")
		...
		for (Map<String, Object> map : list) {
			...
			postMap.put("ordId",
					Integer.parseInt(map.get("order_code").toString()));//订单编号
			postMap.put(
					"ordSeq",
					Integer.parseInt(map.get("order_seq") != null ? map.get(
							"order_seq").toString() : "0"));//订单序号
			postMap.put("goodId",
					Integer.parseInt(map.get("product_code").toString()));//商品编码
			postMap.put("rtnReason", map.get("reason")!=null?map.get("reason").toString():"");//退货原因
			String sku_code = map.get("sku_code") != null ? map.get("sku_code")
					.toString() : "";//SKU编号
			...
			try {
				if (sku_code != null) {
					MDataMap skuDetail = DbUp.upTable("pc_skuinfo").one(
							"sku_code", sku_code);
					if (skuDetail != null) {
						String skuColor = skuDetail.get("sku_key");
						String[] colors = skuColor.split("&");
						Integer colorId = Integer.parseInt(colors[0].replace(
								"color_id=", ""));
						Integer styleId = Integer.parseInt(colors[1].replace(
								"style_id=", ""));
			...
			String ordSeqStr = map.get("order_seq") != null ? map.get(
					"order_seq").toString() : "0";
			if ("0".equals(ordSeqStr) && postMap.get("colorId") == null) {
				errList.add(map.get("order_code").toString());
				continue;
			}
			postMap.put("doType", "01");
		...
		String result = HttpUtil.postJson(url,
				JSONArray.toJSONString(postParams));
		...
		while (it.hasNext()) {
			...
			String saleCodeApp = jsonObject.getString("saleCodeApp") != null ? jsonObject
					.getString("saleCodeApp") : "";
			String saleCodeLd = jsonObject.getString("saleCodeLd") != null ? jsonObject
					.getString("saleCodeLd") : "";
			String subCode = jsonObject.getString("subCode") != null ? jsonObject
					.getString("subCode") : "";
			...
			if ("0".equals(subCode)) {// 此条数据处理成功
				DbUp.upTable("oc_after_sale_ld").dataUpdate(afterSaleMap, "",
						"after_sale_code_app");
			} else {
				afterSaleMap.put("after_sale_status", "00");// 设置成推送
				DbUp.upTable("oc_after_sale_ld").dataUpdate(afterSaleMap, "",
						"after_sale_code_app");// 设置成为异常单，并且不再推送
			}
		}
	}
}

...
* <p>Description:</p>
*/
public class JobSubtractPredictCoinsForCancelOrder extends RootJobForExec {
	...
	public IBaseResult execByInfo(String sInfo) {
		...
		// 家有客代号
		String custId = new HjycoinService().getCustId(orderInfo.get("buyer_code"));
		if(StringUtils.isBlank(custId)){
			mWebResult.setResultCode(0);
			mWebResult.setResultMessage("未查询到家有客代号，无法还原惠币");
			return mWebResult;
		}
		...
		if(giveMoney == null || giveMoney.compareTo(BigDecimal.ZERO) <= 0){
			mWebResult.setResultCode(1);
			mWebResult.setResultMessage("没有惠币需要还原");
			return mWebResult;
		}
		...
		rootResult = new HjycoinService().changeForHjycoin(UpdateCustAmtInput.CurdFlag.ZDHB, giveMoney, custId, orderInfo.get("big_order_code"), sInfo,"20");
	...
	static {
		config.setExecType("449746990036");
	}

...
/**
 * 分销商品签收 type:449746990028
 * 
 * @author 周恩至
 *
 */
public class JobForAgentReceiveOrder extends RootJobForExec {
	...
	public IBaseResult execByInfo(String orderCode) {
		...
		List<Map<String, Object>> agentOrderInfos = DbUp.upTable("fh_agent_order_detail").dataSqlList(sql,
		...
		for (Map<String, Object> info : agentOrderInfos) {
			...
			// 计算分销收益
			BigDecimal profit_money_dec =  new BigDecimal(agentOrderInfo.get("sku_num")).multiply(new BigDecimal(agentOrderInfo.get("profit_money"))
					.multiply(new BigDecimal(agentOrderInfo.get("agent_profit_rate"))))
					.setScale(1, BigDecimal.ROUND_HALF_UP);
			// 计算分销人父级收益
			BigDecimal agent_parent_rate_dec =  new BigDecimal(agentOrderInfo.get("sku_num")).multiply(new BigDecimal(agentOrderInfo.get("profit_money"))
					.multiply(new BigDecimal(agentOrderInfo.get("agent_parent_rate"))))
					.setScale(1, BigDecimal.ROUND_HALF_UP);
	...
	private void caculate(String memberCode, BigDecimal rate, int type, String orderCode) {
		...
		String sql = "UPDATE familyhas.fh_agent_member_info SET predict_money = predict_money -" + rate
	...
	static {
		config.setExecType("449746990028");
	}

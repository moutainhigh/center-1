 ...
 *
 */
public class JobForAgentReturnOrderComplate extends RootJobForExec {
	...
	private MWebResult operateTV(Map<String, Object> returnInfoTV,String returnCode) {
		...
		BigDecimal predict_money_cut_fan =  new BigDecimal(agentOrder.get("agent_parent_rate")).multiply(new BigDecimal(agentOrder.get("profit_money"))).multiply(new BigDecimal(num));
		...
		this.caculate(member_code_parent, predict_money_cut_fan, 2, order_code);
	...
	static {
		config.setExecType("449746990027");
	}

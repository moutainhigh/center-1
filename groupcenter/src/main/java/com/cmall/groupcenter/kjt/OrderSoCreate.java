...
import com.cmall.groupcenter.kjt.config.RsyncConfigOrderSoCreate;
import com.cmall.groupcenter.kjt.request.RsyncRequestOrderSoCreate;
import com.cmall.groupcenter.kjt.response.RsyncResponseOrderSoCreate;
import com.cmall.groupcenter.kjt.response.RsyncResponseOrderSoCreate.Data;
...
public class OrderSoCreate extends RsyncKjt<RsyncConfigOrderSoCreate, RsyncRequestOrderSoCreate, RsyncResponseOrderSoCreate>{
	...
	public RsyncResult doProcess(RsyncRequestOrderSoCreate tRequest,RsyncResponseOrderSoCreate tResponse) {
		...
		if(!"0".equals(tResponse.getCode())) {
			DbUp.upTable("oc_order_kjt_list").dataUpdate(new MDataMap("order_code_seq",order_code,"rsync_desc",desc), "rsync_desc", "order_code_seq");
			
			//{"Code":"2","Desc":"您本月累计订单的数量超出了海关关于个人物品自用、合理数量的规定。请更换实名信息再购买！"}
			if(StringUtils.startsWith(desc, "您本月累计订单的数量超出了海关关于个人物品自用")){
				
				String order_code_ori=(String)DbUp.upTable("oc_order_kjt_list").dataGet("order_code", "order_code_seq=:order_code_seq", new MDataMap("order_code_seq",order_code));
				DbUp.upTable("oc_order_kjt_list").dataUpdate(new MDataMap("order_code_seq",order_code,"sostatus","65","local_status",""), "sostatus,local_status", "order_code_seq");
				creatReturnMoney(order_code,order_code_ori);
				
				MDataMap addressInfo=DbUp.upTable("oc_orderadress").oneWhere("auth_idcard_number", "", "order_code=:order_code", "order_code",order_code_ori);
				String auth_idcard_number=addressInfo.get("auth_idcard_number");
				MDataMap orderInfo=DbUp.upTable("oc_orderinfo").oneWhere("buyer_code,order_status", "", "order_code=:order_code", "order_code",order_code_ori);
				String buyer_code=orderInfo.get("buyer_code");
				String order_status=orderInfo.get("order_status");
				
				DbUp.upTable("mc_authenticationInfo").dataUpdate(new MDataMap("member_code",buyer_code,"idcard_number",auth_idcard_number,"customs_status","1"), "customs_status", "member_code,idcard_number");
				
				 List<Map<String, Object>> klist=DbUp.upTable("oc_order_kjt_list").dataSqlList("SELECT rsync_desc from oc_order_kjt_list where order_code=:order_code GROUP BY rsync_desc",new MDataMap("order_code",order_code_ori));
				 if(klist.size()==1&&!"4497153900010006".equals(order_status)){
					 //更新订单状态
					 DbUp.upTable("oc_orderinfo").dataUpdate(new MDataMap("order_status","4497153900010006","order_code",order_code_ori), "order_status", "order_code");
					 DbUp.upTable("lc_orderstatus").insert("code",order_code_ori,"info","跨境通订单同步失败","create_time",DateUtil.getSysDateTimeString(),"create_user","system","old_status",order_status,"now_status","4497153900010006");
				 }
			}
		...
		//更新订单表
		DbUp.upTable("oc_order_kjt_list").dataUpdate(new MDataMap("order_code_seq",order_code,"order_code_out",out_order_code,"product_amount",ProductAmount,"tax_amount",TaxAmount,"shipping_amount",ShippingAmount,"update_time",DateUtil.getSysDateTimeString(),"rsync_desc",desc,"sostatus","0"), "order_code_out,product_amount,tax_amount,shipping_amount,update_time,rsync_desc,sostatus", "order_code_seq");

...
import com.cmall.groupcenter.kjt.config.RsyncConfigGetKjtProductIdByDate;
import com.cmall.groupcenter.kjt.model.RsyncModelGetKjtChangeProduct;
import com.cmall.groupcenter.kjt.request.RsyncRequestGetKjtProductIdByDate;
import com.cmall.groupcenter.kjt.response.RsyncResponseGetKjtProductIdByDate;
 ...
 * 
 */
public class RsyncGetKjtProductIdByDate 
		extends
	...
	public RsyncRequestGetKjtProductIdByDate upRsyncRequest() {
		...
		request.setSaleChannelSysNo(bConfig("groupcenter.rsync_kjt_SaleChannelSysNo"));
		request.setLimitRows(bConfig("groupcenter.rsync_kjt_limit_rows"));//默认足够大的条数1百万
		return request;
	}
				...
				if(dowProductCodeOldList!= null && dowProductCodeOldList.size() > 0){
					...
					if(downProductList != null && downProductList.size() > 0){
						...
						if(pCode1 != null && pCode1.size() > 0){
							String sWhere = "product_code_old in ('"+ StringUtils.join(pCode1, "','")+"')  and small_seller_code='SF03KJT' and seller_code='SI2003'";
							List<MDataMap> downProductCode = DbUp.upTable("pc_productinfo").queryAll("uid,product_code", "", sWhere, new MDataMap());
							FlowBussinessService fbs = new FlowBussinessService();
							for (MDataMap downProductMap : downProductCode) {
								String flowBussinessUid=downProductMap.get("uid");
								String fromStatus= "4497153900060002";
								String toStatus="4497153900060003";
								String flowType = "449715390006";
								String userCode = "jobsystem";
								String remark="同步的跨境通商品为下架状态，将系统中的商品下架";
//								RootResult ret =
								fbs.ChangeFlow(flowBussinessUid, flowType, fromStatus, toStatus, userCode, remark, new MDataMap());
							}
						}
					}
				}

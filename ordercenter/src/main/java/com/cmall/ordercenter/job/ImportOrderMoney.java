 ...
 * 描述: 定时任务：同步外部导入订单相关金额到惠家有订单表中 <br>
 * 作者: zhy<br>
 * 时间: 2016-09-23
 */
public class ImportOrderMoney extends RootJob {
	...
	public void doExecute(JobExecutionContext context) {
		try {
			 ...
			 * 查询导入的外部订单集合
			 */
			List<Map<String, Object>> list = DbUp.upTable("oc_orderinfo_import").dataSqlList(
					"select * from ordercenter.oc_orderinfo_import where  is_sync=0 and import_status='4497479500010002' ",
					null);
			if (list != null && list.size() > 0) {
				for (Map<String, Object> importOrder : list) {
					MDataMap map = new MDataMap(importOrder);
					// 商品总金额
					...
					if (StringUtils.isNotBlank(map.get("oc_order_code"))) {
						...
						if (resultOrder >= 0) {
							...
							DbUp.upTable("oc_orderinfo_import").dataUpdate(editBdwm, "is_sync,remark", "zid");
						} else {
							// 添加错误日志到百度外卖订单表
							MDataMap editBdwm = new MDataMap();
							editBdwm.put("zid", map.get("zid"));

...
import com.cmall.ordercenter.model.api.ApiOrderStatusChangeNoticResult;
 ...
 * @author renhongbin
 */
public class ApiOrderStatusChangeNotic extends RootApi<ApiOrderStatusChangeNoticResult, ApiOrderStatusChangeNoticInput>{

	public ApiOrderStatusChangeNoticResult Process(ApiOrderStatusChangeNoticInput inputParam, MDataMap mRequestMap) {

		Date request_time = new Date();
		
		ApiOrderStatusChangeNoticResult result = new ApiOrderStatusChangeNoticResult();
		...
		try {
			...
			mDataMap.put("status", inputParam.getStatus());
			mDataMap.put("statusCode", inputParam.getStatusCode());
			mDataMap.put("updateTime", inputParam.getUpdateTime());
			...
			String url = bConfig("ordercenter.dcb_interface_url") + "online-shop/_3rd/huijiayou/order_state_change";

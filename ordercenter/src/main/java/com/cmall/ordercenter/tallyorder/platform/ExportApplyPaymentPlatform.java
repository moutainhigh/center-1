 ...
 * 
 */
public class ExportApplyPaymentPlatform extends RootExport {
	...
	public void export(String sOperateId, HttpServletRequest request,
		...
		String where = "";
		String sql = "select pay_code, settle_codes, merchant_code, merchant_name, period_collect_amount_total, "
				+ "service_fee, payable_collect_amount, add_deduction, settle_collect_amount, period_money, actual_pay_amount,create_time, pay_time "
				+ "from oc_bill_apply_payment_pt a where 1=1 ";
...
//		FileOutputStream out = null;
		try {
			in = ExportApplyPaymentPlatform.class
					.getResourceAsStream("/applyPaymentPlatform.xls");
...
//			out = new FileOutputStream(sdf.format(new Date())); 
			...
			if(null != applyPayList && applyPayList.size() > 0) {
				for (Map<String, Object> applyPay : applyPayList) {
					...
					//结算单号
					String settleCodes = applyPay.get("settle_codes").toString();
					//商户编号
					String sellerCode = applyPay.get("merchant_code").toString();
					//商户名称
					String sellerName = applyPay.get("merchant_name").toString();
					//本期代收货款合计
					double periodCollectAmountTotal = Double.valueOf(isEmpty(applyPay.get("period_collect_amount_total")) ? "0.00"
							: applyPay.get("period_collect_amount_total").toString());
					//服务费
					double serviceFee = Double.valueOf(isEmpty(applyPay.get("service_fee")) ? "0.00"
							: applyPay.get("service_fee").toString());
					//应付代收货款
					double payableCollectAmount = Double.valueOf(isEmpty(applyPay.get("payable_collect_amount")) ? "0.00"
							: applyPay.get("payable_collect_amount").toString());
					//附加扣费合计
					...
					row.createCell(1).setCellValue(settleCodes);	 //结算单号
					row.createCell(2).setCellValue(sellerCode); 	 //商户编号
					row.createCell(3).setCellValue(sellerName);		 //商户名称
					row.createCell(4).setCellValue(periodCollectAmountTotal);   //本期代收货款合计
					row.createCell(5).setCellValue(serviceFee);   		   //服务费
					row.createCell(6).setCellValue(payableCollectAmount);  //应付代收货款
					row.createCell(7).setCellValue(addDeduction);          //附加扣费合计
					row.createCell(8).setCellValue(settleCollectAmount);   //结算代收货款
					row.createCell(9).setCellValue(periodMoney);	       //本期质保金
					row.createCell(10).setCellValue(actualPayAmount); 	   //实付代收货款
					row.createCell(11).setCellValue(createTime);		   //创建时间
					row.createCell(12).setCellValue(payTime);			   //付款日期
					startrow++;
				}
			}
//			wb.write(out);
			...
			response.setHeader("Content-disposition", "attachment;filename=" + 
					java.net.URLEncoder.encode("平台入驻-导出申请单-" + sdf.format(new Date()), "UTF-8").replace("+", "")  + ".xls");

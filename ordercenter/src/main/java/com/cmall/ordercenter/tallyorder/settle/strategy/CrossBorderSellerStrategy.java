 ...
 *
 */
public class CrossBorderSellerStrategy extends SettleStrategy {

	public CrossBorderSellerStrategy(Map<String, Object> settlePeriod) {
		super(settlePeriod);
	}
	...
	protected List<String> getSuccessSellerList() {
		...
		try {
			...
			// 只结算跨境商户
			String settleTypeWhere = " AND si.uc_seller_type='4497478100050002' ";
			...
			if (pcExtMap != null) {
				...
				if (settlement_type.equals("4497471600110003") && purchase_type.equals("4497471600160003")) {
					...
					DbUp.upTable("oc_bill_final_export_tmp").insert("settle_code", getSettleCode(), "order_code",
							order_code, "product_settle_type", "跨境商户结算", "passage", seller_code, "product_code",
		...
		if (list.size() > 0) {
			for (Map<String, Object> mapinfo : list) {
				...
				DbUp.upTable("oc_bill_final_export").insert("settle_code", getSettleCode(), "product_settle_type",
						"跨境商户结算", "passage", passage, "product_code", product_code, "product_name", product_name,
			...
			for (Map<String, Object> mapinfo : list) {
				...
				if (StringUtils.isNotEmpty(small_seller_code)) {
					DbUp.upTable("oc_bill_finance_amount").insert("settle_code", getSettleCode(), "settle_type",
							...
							"current_period_money", String.valueOf(current_period_money),"service_fee",
							String.valueOf(service_fee), "settle_pay_moeny", String.valueOf(actual_pay_amount),

...
/** 
 * 商户结算5.0-平台入驻付款申请单财务审核-通过(将付款申请单状态改为财务审核通过)
 * 审核平台入驻商户付款申请单状态
 ...
 * @author zht
 */
public class UpdatePayForAuditStatusToPassPlatform extends RootFunc{
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		DbUp.upTable("oc_bill_apply_payment_pt").dataUpdate(new MDataMap("uid",uid,"flag","4497477900060004"),"flag", "uid");
		//记日志
		Date now = new Date();
		...
		try{
			if (mResult.upFlagTrue()) {
				...
				logMap.put("ip", ip);
				logMap.put("comment", "平台入驻付款申请单财务审核通过");

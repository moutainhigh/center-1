...
/**
 * 商户结算5.0-平台入驻付款申请单财务审核-驳回(将付款申请单状态改为财务审核驳回)
 * 审核平台入驻商户付款申请单状态
 ...
 *
 */
public class UpdatePayForAuditStatusToRejectPlatform extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		DbUp.upTable("oc_bill_apply_payment_pt").dataUpdate(new MDataMap("uid",uid, "flag","4497477900060005", "reject_reason",rejectReason), "flag,reject_reason", "uid");
		//记日志
		Date now = new Date();
		...
		try{
			if (mResult.upFlagTrue()) {
				...
				logMap.put("ip", ip);
				String comment = "平台入驻付款申请单财务驳回" + (StringUtils.isNotBlank(rejectReason) ? " 备注:" + rejectReason : "");
				logMap.put("comment", "平台入驻付款申请单财务审核被驳回");
...
//		mResult.setResultMessage("付款申请单[" + payCode + "]审批拒绝!");
		return mResult;
	}
}

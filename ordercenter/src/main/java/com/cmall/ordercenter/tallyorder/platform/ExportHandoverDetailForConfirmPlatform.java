...
public class ExportHandoverDetailForConfirmPlatform extends RootExport {
	...
	public void export(String sOperateId, HttpServletRequest request,
		...
		String sql = "select merchant_code, merchant_name, actual_pay_amount, "
				+ "b.branch_name, b.joint_number, b.branch_address, b.bank_account "
				+ "from ordercenter.oc_bill_apply_payment_pt a inner join usercenter.uc_seller_info_extend b "
				+ "where a.merchant_code =b.small_seller_code and a.settle_type='4497477900040004' "; 
	...
	public void handOverToExcel(String sql, HttpServletResponse response) throws Exception {
		...
		try {
			in = ExportHandoverDetailForConfirmPlatform.class.getResourceAsStream("/handoverDetailPlatformForPay.xls");
			...
			if(null != applyPayList && applyPayList.size() > 0) {
				...
				for (Map<String, Object> applyPay : applyPayList) {
					...
					row.createCell(6).setCellValue(branchAddress);      //开户行支行所在地
					row.createCell(7).setCellValue("联行号:" + jointNumber);   //联行号
				}
				row = sheet.createRow((short) ++startrow); 	 	 
				row.createCell(0).setCellValue("合计");
				row.createCell(5).setCellValue(sumActualPayAmount);	 //累加实付代收货款
				row = sheet.createRow((short) ++startrow); 
			...
			response.setHeader("Content-disposition", "attachment;filename=" +
					java.net.URLEncoder.encode("平台入驻-付款确认-导出付款申请单", "UTF-8").replace("+", "") + ".xls");

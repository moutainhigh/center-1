...
/**
 * 商户结算3.0-付款申请单审核-拒绝(将付款申请单状态改为已拒绝)
 ...
 *
 */
public class UpdatePayForAuditStatusToReject extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		DbUp.upTable("oc_bill_apply_payment").dataUpdate(new MDataMap("uid",uid, "flag","4497477900010005", "reject_reason",rejectReason), "flag,reject_reason", "uid");
		//记日志
		Date now = new Date();
		...
		try{
			if (mResult.upFlagTrue()) {
				...
				logMap.put("flag", "4497477900010005");
				logMap.put("ip", ip);

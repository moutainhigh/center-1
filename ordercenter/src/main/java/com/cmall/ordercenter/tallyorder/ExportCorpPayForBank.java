 ...
 */
public class ExportCorpPayForBank extends RootExport {
	...
	public void export(String sOperateId, HttpServletRequest request, HttpServletResponse response) {
		...
		//只导出已确认的付款申请单
		String where = "";
		String sql = "select a.pay_code as pay_code, a.merchant_code as merchant_code, a.merchant_name as merchant_name, a.invoice_amount as invoice_amount, "
				+ "a.actual_invoice_amount as actual_invoice_amount, a.period_money as period_money, a.add_deduction as addContent_deduction, a.actual_pay_amount as actual_pay_amount, "
				+ "a.wait_pay_amount as wait_pay_amount, a.create_time as create_time, a.pay_time as pay_time, b.bank_account as bank_account, b.branch_name as branch_name "
				+ "from ordercenter.oc_bill_apply_payment a, usercenter.uc_seller_info_extend b "
				+ "where a.flag= '4497477900010003' and a.merchant_code=b.small_seller_code and a.wait_pay_amount>0.0 and (a.is_pay='4497477900020002' OR a.is_pay is null) ";
...
//		String creator = UserFactory.INSTANCE.create().getLoginName();
		...
		String wait_pay_amount = ObjectUtils.defaultIfNull(applyPay.get("wait_pay_amount"), new BigDecimal(0.0)).toString();
		try {
			Double.parseDouble(wait_pay_amount);
		} catch(NumberFormatException e) {
			wait_pay_amount = "0.0";
		}
		original_sum.setText(wait_pay_amount);
		...
		local_sum.setText(wait_pay_amount);
	...
	private Element createBody(String nodeName, Map<String, Object> applyPay) throws Exception {
		...
		digest.setText("支付供应商" + sellerName + "[" + smallSellerCode  + "]" + "常规结算货款");
		...
		debit_original_sum.setText(wait_pay_amount);
		...
		local_balance.setText(wait_pay_amount);
		...
		debit_local_noTax.setText(wait_pay_amount);
		...
		debit_original_noTax.setText(wait_pay_amount);

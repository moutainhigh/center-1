...
/**
 * 
 * 类: ExportCorpPayForBankKj <br>
 * 描述: 跨境相关-导出银企直连文件 <br>
 * 待审核 4497477900010001<br>
 * 审核通过 4497477900010002<br>
 * 已确认 4497477900010003<br>
 * 已付款 4497477900010004<br>
 * 拒绝 4497477900010005<br>
 * 已收款 4497477900010006<br>
 * 作者: zhy<br>
 * 时间: 2017年5月3日 下午2:14:17
 */
public class ExportCorpPayForBankKj extends RootExport {
	...
	public void export(String sOperateId, HttpServletRequest request, HttpServletResponse response) {
		...
		// 只导出已确认的付款申请单
		String where = "";
		String sql = "SELECT pay_code,merchant_code,merchant_name,period_collect_amount_total,";
		sql += " service_fee,payable_collect_amount,add_deduction,settle_collect_amount, ";
		sql += " period_money,actual_pay_amount,settle_collect_amount,period_money, ";
		sql += " actual_pay_amount,branch_name,joint_number,branch_address,bank_account,create_time,pay_time ";
		sql += " FROM oc_bill_apply_payment_kj AS a WHERE a.flag = '4497477900060006' ";
			...
			if (null != applyPayList && applyPayList.size() > 0) {
				for (Map<String, Object> applyPay : applyPayList) {
					...
					try {
						...
						String payCode = StringUtils.isEmpty((String) applyPay.get("pay_code")) ? ""
								: applyPay.get("pay_code").toString();
	...
	private Element createHeader(String nodeName, Map<String, Object> applyPay) {
		...
		String comment = (StringUtils.isEmpty((String) applyPay.get("pay_code")) ? ""
				: applyPay.get("pay_code").toString());
		scomment.setText(comment);
		header.addContent(scomment);

		// 原币金额(待付款金额:period_collect_amount_total)
		Element original_sum = new Element("original_sum");
		String actual_pay_amount = ObjectUtils
				.defaultIfNull(applyPay.get("actual_pay_amount"), new BigDecimal(0.0)).toString();
		try {
		...
		// 本币金额(待付款金额:payable_collect_amount)
	...
	private Element createBody(String nodeName, Map<String, Object> applyPay) throws Exception {
		...
		String sellerName = StringUtils.isEmpty((String) applyPay.get("merchant_name")) ? ""
				: applyPay.get("merchant_name").toString();
		String smallSellerCode = StringUtils.isEmpty((String) applyPay.get("merchant_code")) ? ""
				: applyPay.get("merchant_code").toString();
		digest.setText("支付供应商" + sellerName + "[" + smallSellerCode + "]" + "跨境代收货款");
		...
		customer.setText(StringUtils.isEmpty((String) applyPay.get("merchant_name")) ? ""
				: applyPay.get("merchant_name").toString());
		...
		String payable_collect_amount = ObjectUtils
				.defaultIfNull(applyPay.get("payable_collect_amount"), new BigDecimal(0.0)).toString();
		try {
			Double.parseDouble(payable_collect_amount);
		} catch (NumberFormatException e) {
			payable_collect_amount = "0.0";
		}
		debit_original_sum.setText(payable_collect_amount);
		...
		debit_local_sum.setText(payable_collect_amount);
		entry.addContent(debit_local_sum);

		// 原币余额
		Element original_balance = new Element("original_balance");
		original_balance.setText(payable_collect_amount);
		...
		local_balance.setText(payable_collect_amount);
		...
		debit_local_noTax.setText(payable_collect_amount);
		...
		debit_original_noTax.setText(payable_collect_amount);
		...
		pay.setText(payable_collect_amount);
		settlement.addContent(pay);

		// 本币付款
		Element paylocal = new Element("paylocal");
		paylocal.setText(payable_collect_amount);

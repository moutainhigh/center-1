 ...
 *
 */
public class PlatformSettleStrategy extends SettleStrategy {

	public PlatformSettleStrategy(Map<String, Object> settlePeriod) {
		super(settlePeriod);
	}
	...
	protected List<String> getSuccessSellerList() {
		...
		try {
			...
			// 只结算平台商户
			String settleTypeWhere = " AND si.uc_seller_type='4497478100050004' ";
			...
			String sql = "SELECT " + "small_seller_code " + "FROM " + "usercenter.uc_seller_info_extend si " + "WHERE "
					...
					+ "now_status=:now_status " + "AND create_time>=:create_time_from AND zid > 114806056 "
		...
		try {
			if (!isTestSeller(smallSellerCode)) {
				...
				String sql = "SELECT " + "a.code, a.create_time " + "FROM " + "(" + "SELECT	code, create_time "
						...
						// + "AND b.pay_type=:pay_type";
						//+ "AND b.pay_type in ('449716200001','449716200005','449716200006','4497162000070001') ";
						+ "AND b.pay_type in ("+getPayType()+") ";
		...
		for (Map<String, Object> map2 : list) {
			...
			if (pcExtMap != null) {
				...
				if (settlement_type.equals("4497471600110003") && purchase_type.equals("4497471600160003")) {
					...
					// 获取商户的信息
					// 新增查询字段quality_retention_money（最大质保金）,money_proportion（质保金比例）,money_collection_way（质保金收取方式）
					// 2016-10-28 zhy
					...
					String money_collection_way = "账扣";// 质保金收取方式
					...
					DbUp.upTable("oc_bill_final_export_tmp").insert("settle_code", getSettleCode(), "order_code",
							order_code, "product_settle_type", "平台入驻结算", "passage", seller_code, "product_code",
							...
							returnEnd,
							// 新增添加最大质保金，质保金比例，质保金收取方式 2016-10-28 zhy
			...
			if (!isTestSeller(smallSellerCode)) {
				...
				if (null != returnList && returnList.size() > 0) {
					for (Map<String, Object> returnItem : returnList) {
						//map.put(returnItem.get("order_code").toString(), returnItem.get("create_time").toString());
//						map.put(returnItem.get("order_code").toString(), returnItem.get("return_code").toString()+","+returnItem.get("create_time").toString());
	...
	private void genSkuReturnedSettleInfo(MDataMap map, String returnStart, String returnEnd, String saleStart,
		...
		// from oc_orderdetail where order_code " + ordersql;
		// 部分退换货sql
		String skuSql = "SELECT a.return_code, a.order_code AS order_code, b.sku_code AS sku_code, sum(b.count) AS count,	b.return_code,b.current_price "
				...
				+"and a.return_code "+returnCodeSql
				+ " group by a.order_code, b.return_code, b.sku_code";
	...
	protected void summarizeSkuSettleInfo() {
		...
		sb.append(",money_collection_way,max_retention_money,rate,money_proportion_rate");
		...
		if (list.size() > 0) {
			for (Map<String, Object> mapinfo : list) {
				...
				DbUp.upTable("oc_bill_final_export").insert("settle_code", getSettleCode(), "product_settle_type",
						"平台入驻结算", "passage", passage, "product_code", product_code, "product_name", product_name,
						...
						// 质保金比例
						"money_proportion_rate", String.valueOf(money_proportion_rate),
						// 已扣质保金
						"deduct_retention_money", String.valueOf(deduct_retention_money),
						// 本期质保金
						"period_retention_money", String.valueOf(period_retention_money),
						// 质保金收取方式
			...
			for (Map<String, Object> mapinfo : list) {
				...
				if (StringUtils.isNotEmpty(small_seller_code)) {
					DbUp.upTable("oc_bill_finance_amount").insert("settle_code", getSettleCode(), "settle_type",
							...
							String.valueOf(related_charges), "service_fee", String.valueOf(service_fee),
							"settle_pay_moeny", String.valueOf(actual_pay_amount), "start_time", saleStart, "end_time",
							saleEnd, "settle_period", saleStart.substring(0, 10) + "至" + saleEnd.substring(0, 10),
							"settle_status", "1", "tuistart", returnStart, "tuiend", returnEnd, "small_seller_code",
							small_seller_code);
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		for (Map<String, Object> period : periodList) {
			if ("08".equals(period.get("code").toString())) {
				PlatformSettleStrategy s = new PlatformSettleStrategy(period);

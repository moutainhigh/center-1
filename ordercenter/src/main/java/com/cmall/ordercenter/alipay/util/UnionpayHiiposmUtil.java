...
public class UnionpayHiiposmUtil {
	public boolean MD5Verify(String srcData, String hmac, String key) {
		String _crpy = MD5Sign(srcData, key);
		return _crpy.equals(hmac);
	}

	public String sendAndRecv(String url, String buf, String characterSet)
			throws IOException {
		String charType = characterSet;
		if ("00".equals(characterSet)) {
			charType = "GBK";
		} else {
			if ("01".equals(characterSet)) {
				charType = "GB2312";
			} else {
				if ("02".equals(characterSet))
					charType = "UTF-8";
				else
					charType = null;
			}
		...
		for (int i = 0; i < aParam.length; i++) {
			z = aParam[i].indexOf('=');
			if (z != -1) {
				pm.addParameter(aParam[i].substring(0, z++),
						aParam[i].substring(z));
			}
	...
	public String MD5Sign(String signData, String signkey) {
		UnionpayCryptUtilImpl impl = new UnionpayCryptUtilImpl();

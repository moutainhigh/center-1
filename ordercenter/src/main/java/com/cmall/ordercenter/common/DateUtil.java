...
public class DateUtil {
	...
	/**
	 * 利用指定String formatter转换String到String
	 * @param sDate
	 * 				String sDate
	 * @param formatter
	 * 				String formatter
	 ...
	 * @history
	 */
	public synchronized static String toFormatDate(String sDate, String formatter) {
		String sRet = null;
		if(sDate == null || "".equals(sDate)) return null;
		try {
			java.util.Date date = DateUtil.toDate(sDate);
			sRet = DateUtil.toString(date, formatter);
		} catch (Exception e) {
			e.printStackTrace();
			sRet = null;
		}
...
//		System.out.print(date1.compareTo(date2));
		//System.out.println(getMonth(null));
	}
	 ...
	 * @return
	 */
	public static String getTimeCompareSomeDay(int days){
		SimpleDateFormat sf  =new SimpleDateFormat(DateUtil.DATE_FORMAT_DATETIME);
	 ...
	 */
	public synchronized static long subtime(String start,String end,String pattern) throws ParseException{
	...
	* @param formart 时间格式
	* @author 张海生
	* @date 2015-5-17 下午4:45:00
	* @return  前者小于后者小于0,大于时大于0,相等为0
	* @throws 
	*/
	public synchronized static int compareTime(String start, String end, String formart){
		int flag = 0;
		try {
			java.util.Date date1 = convertToDate(start,formart);
			java.util.Date date2  = convertToDate(end,formart);
	...
	/**
	 * @Description:比较两个日期时间的大小，时间格式可为yyyy-MM-dd HH:mm:ss
	 * @param startTime
	 * @param endTime
	 * @return
	 */

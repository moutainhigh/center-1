...
public class ForgetAgentPassword extends
	...
	public RootResultWeb Process(ForgetInput inputParam, MDataMap mRequestMap) {
		...
		if (getFlagLogin()) {
			...
			rootResultWeb.inOtherResult(new MemberInfoSupport().forgetAgentPassword(
			...
			if (rootResultWeb.upFlagTrue()) {
				...
				if (rootResultWeb.upFlagTrue()) {
					rootResultWeb.inOtherResult(new MemberInfoSupport()
							.forgetAgentPassword(inputParam.getLogin_name(),

 ...
 * 
 */
public class FuncAddForSystemMessage extends RootFunc {
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		//mInsertMap.put("manage_code", bConfig("membercenter.member_default_appcode"));
		if (mResult.upFlagTrue()) {
			
			DbUp.upTable(mPage.getPageTable()).dataInsert(mInsertMap);
			
			/**如果是系统消息*/
			if(mInsertMap.get("message_type").equals("449746640001")){
			...
			if(list.size()!=0){
				...
				for(int i=0;i<list.size();i++){
					...
					messDataMap.put("manage_code", mInsertMap.get("manage_code"));

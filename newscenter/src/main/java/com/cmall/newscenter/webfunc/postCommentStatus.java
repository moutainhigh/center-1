 ...
 *
 */
public class postCommentStatus extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		}else if("449746800002".equals(isDisable)){
			dataMap.put("status", "449746800001");
		//	dataMap.put("disableDate", FormatHelper.upDateTime());
			DbUp.upTable(TABLE_TPL).dataUpdate(dataMap, "status", "uid");
			mLogDataMap.put("operate_status","449747020001"); //审核通过：449747020001； 审核拒绝：449747020002；敏感词过滤： 449747020001
			DbUp.upTable("nc_comment_log").dataInsert(mLogDataMap);
		}else{
			mResult.setResultCode(934205104);
			mResult.setResultMessage("已是审核通过状态！");
		}

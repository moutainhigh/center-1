...
import com.cmall.newscenter.beauty.model.ApiOrderListInput;
import com.cmall.newscenter.beauty.model.ApiOrderListResult;
import com.cmall.newscenter.beauty.model.ApiSellerListResult;
import com.cmall.newscenter.beauty.model.ApiSellerOrderListResult;
import com.cmall.newscenter.beauty.model.ApiSellerStandardAndStyleResult;
...
public class ApiOrderList extends RootApiForToken<ApiOrderListResult, ApiOrderListInput>{
	...
	public ApiOrderListResult Process(ApiOrderListInput inputParam,
		...
		if(!"".equals(inputParam.getBuyer_code()) 
			...
			count = orderService.orderCountForBeauty(inputParam.getBuyer_code(),inputParam.getOrder_status());   //统计订单总数
			if(count !=0){
				 countPage = (count/10)+1;    //总页数
				 int size = count/10;//总条数/每页显示的条数=总页数
		         int mod = count % 10;//最后一页的条数
		          if(mod != 0){
		        	  size++; 
		          }
		         countPage = size;
			...
			orderList = orderService.orderInformationForBeauty(inputParam.getBuyer_code(), inputParam.getOrder_status(), inputParam.getNextPage());   //订单信息
			if(orderList!=null && !orderList.isEmpty()){
				for(int i=0; i<orderList.size(); i++){
					...
					if(BigDecimal.ZERO.compareTo(BigDecimal.valueOf(Double.valueOf(map.get("due_money").toString())))==0){
						...
						for(int j=0; j<orderSellerList.size(); j++){
							...
							if(sellerList != null && !sellerList.isEmpty()){
								for(int k=0; k<sellerList.size(); k++){
									...
									apiSellerListResult.setProductType(String.valueOf(productService.getSkuActivityType(sellerMap.get("sku_code").toString(),getManageCode())));   //商品类型
									if(productService.getSkuActivityType(sellerMap.get("sku_code").toString(),getManageCode())==2){
										apiSellerListResult.setProductType("0");							
									}
									boolean isCommented = productCommentService.isCommented(map.get("order_code").toString(),getUserCode(), sellerMap.get("sku_code").toString(), getManageCode());
									if(isCommented){
			...
			if(Integer.parseInt(inputParam.getNextPage())<=countPage){
				apiOrderListResult.setNowPage(Integer.parseInt(inputParam.getNextPage()));
			}else{
				if(countPage!=0){
					apiOrderListResult.setResultCode(0);
					apiOrderListResult.setResultMessage("当前页数大于总页数");
				}
			}

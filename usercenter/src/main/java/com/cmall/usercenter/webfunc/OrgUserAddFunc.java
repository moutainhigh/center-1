...
/**
 * ClassName: 部门下添加人员 <br/>
 * Date:     2013-11-12 下午3:23:41 <br/>
 * @author   jack
 * @version  1.0
 */
public class OrgUserAddFunc extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
				...
				if (StringUtils.isNotEmpty(mField.getFieldScope())) {
					...
					if (mScopeMap.containsKey("targetset")) {
						...
						if (sTargetSetString.equals("unique")) {
							if (DbUp.upTable(mPage.getPageTable()).count(mField.getColumnName(),mInsertMap.get(mField.getColumnName())) > 0) {
						...
						// 自动生成code
							...
							if (mTopDataMap != null) {
								...
								mInsertMap.put(mField.getColumnName(),sParentValue+String.valueOf(lMax));
							} else {
		...
		if (mResult.upFlagTrue()) {
			List<MDataMap> exitList = DbUp.upTable(mPage.getPageTable()).queryAll("", "", "", mInsertMap);
			if(exitList.isEmpty()){
			DbUp.upTable(mPage.getPageTable()).dataInsert(mInsertMap);
				...
				mResult.setResultMessage(bInfo(959701022));
			}
		}

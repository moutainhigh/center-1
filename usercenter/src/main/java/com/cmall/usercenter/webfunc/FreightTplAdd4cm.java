 ...
 *
 */
public class FreightTplAdd4cm extends RootFunc{
	...
	public MWebResult funcDo(String sOperateUid, MDataMap dataMap) {
		...
		try {
			...
			String is_default = u.getIs_default();
			MDataMap mDataMap = new MDataMap();
			mDataMap.put("is_default", is_default);
			if("449746250001".equals(is_default) && "449746250002".equals(u.getIsDisable())) {
				mDataMap.put("isDisable", "449746250002");
				int dataCount = DbUp.upTable(TABLE_TPL).dataCount("is_default =:is_default and isDisable =:isDisable", mDataMap);
				if(dataCount > 0) {
					mResult.setResultCode(959701038);
					mResult.setResultMessage("已存在启用的默认运费模板,不能添加该模板!");
					return mResult;
				}
			}
		...
		mResult.setResultObject("zapjs.f.tourl('page_chart_v_cm_uc_freight_tpl')"); 
		return mResult;
	}

 ...
 * @Description: 添加合同类型（新）
 * @author 李泽帆
 * @date 2017年11月17日11:00:58
 * 
 */
public class ContractFuncNewAdd extends FuncAdd {
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		dataMap.put("zw_f_create_time", create_time);
		dataMap.put("zw_f_create_user", create_user);
		...
		int count = DbUp.upTable("fh_contract_new").count("contract_code",
				contractCode);
		if (count > 0) {
		...
		try {
			DbUp.upTable("fh_contract_new").dataInsert(maddMap);
		} catch (Exception e) {
			e.printStackTrace();

 ...
 * 
 */
public class FuncAddForQualificationApproval extends RootFunc {
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		if (mResult.upFlagTrue()) {
			...
			if (null!=sellerQualification && StringUtils.isNotBlank(smallSellerCode)) {
				...
				//判断是新增待审批状态：4497478200010003，还是修改待审批状态：4497478200010004

				MDataMap qualificationMap = DbUp.upTable("pc_seller_qualification").one("seller_qualification_code",sellerQualification.getSellerQualificationCode());
				String flowStatus  = "";
				String title = "";
				if (null == qualificationMap || qualificationMap.isEmpty()) {
					flowStatus = "4497478200010003";
					title = bInfo(941901139,smallSellerCode);
				}else{
					flowStatus = "4497478200010004";
					title = bInfo(941901140,smallSellerCode);
				}
				//插入到草稿箱表中
				productBrandService.insertPualificationDraft(sellerQualification,flowStatus,"");

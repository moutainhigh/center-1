...
/**
 *商品cps新增
 * Date:     2013-11-5 下午7:14:53 <br/>
 * @author   jack
 */
public class ProductCpsAdd extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		if (mResult.upFlagTrue()) {
			String [] productCodes = mInsertMap.get("product_code").split(",");
			for(int i = 0;i<productCodes.length;i++){
					MDataMap map = new MDataMap();
					map.put("product_code", productCodes[i]);
					...
					if(exit.isEmpty()){
						...
						DbUp.upTable(mPage.getPageTable()).dataUpdate(map, "cpsrate", "product_code");
					}
					map.clear();
			}
			if (bFlagComponent) {

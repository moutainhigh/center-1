...
/**
 *店铺关联的类目cps新增
 * Date:     2013-11-4 下午7:14:53 <br/>
 * @author   jack
 */
public class SellerCategoryCpsAdd extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
				...
				if (StringUtils.isNotEmpty(mField.getFieldScope())) {
					...
					if (StringUtils.isNotEmpty(sDefaultValue)) {
						...
						if (StringUtils.isNotEmpty(sValue)) {
							mDataMap.put(
									WebConst.CONST_WEB_FIELD_NAME+ mField.getFieldName(), sValue);
							mInsertMap.put(mField.getColumnName(), sValue);
						}
					}
					// 如果有附件设置
		...
		if (mResult.upFlagTrue()) {
			String [] categoryCodes = mInsertMap.get("category_code").split(",");
			String [] sellerCodes = mInsertMap.get("seller_code").split(",");
			for(int i = 0;i<sellerCodes.length;i++){
				for(int j=0;j<categoryCodes.length;j++){
					MDataMap map = new MDataMap();
					map.put("seller_code", sellerCodes[i]);
					map.put("category_code", categoryCodes[j]);
					List<MDataMap> exit =	DbUp.upTable(mPage.getPageTable()).queryAll("", "", "", map);
					map.put("cpsrate", mInsertMap.get("cpsrate"));
					if(exit.isEmpty()){
						DbUp.upTable(mPage.getPageTable()).dataInsert(map);
					}else{
						DbUp.upTable(mPage.getPageTable()).dataUpdate(map, "cpsrate", "category_code,seller_code");
					}
					map.clear();
				}
			}

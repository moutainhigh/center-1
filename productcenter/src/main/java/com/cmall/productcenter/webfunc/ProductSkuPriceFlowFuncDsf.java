 ...
 * 
 */
public class ProductSkuPriceFlowFuncDsf extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		try {
			if (mResult.upFlagTrue()) {
				...
				List<MDataMap> list = initDataMap(pcFlows);
				//判读当前的审批流程是否存在。
	 ...
	 * @return
	 */
	public List<MDataMap> initDataMap(List<LinkedHashMap<String, Object>> pcFlows) throws Exception {
		...
		for (Map<String, Object> dataMap : pcFlows) {
			...
			if(costPriceOld.compareTo(costPrice)<0) {
				String startTime = mDataMap.get("start_time").toString();
				String endTime =  mDataMap.get("end_time").toString();
				...
				String sql ="select * from "+ 
						...
						+ " a.sku_code =  '"+skuCode+"') aa "
								+ "where (aa.begin_time < '"+startTime+"' and aa.end_time > '"+startTime+"') "

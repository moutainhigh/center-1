...
public class AddProductSkuForCf extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		try {
			...
			if (mResult.upFlagTrue()) {
				...
				skuinfo.setCostPrice(new BigDecimal(mSubDataMap.get("costPrice")));
				...
				skuinfo.setMiniOrder(Integer.valueOf(mSubDataMap.get("miniOrder")));
...
//				RootResult root = pService.addSku(skuinfo);
				...
				if (null == pro || StringUtils.isBlank(pro.getProductCode())) {
					pro = pService.getProduct(mSubDataMap.get("productCode"));
				}
				...
				pService.updateProduct(pro, error);

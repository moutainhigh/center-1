...
public class ApigetLabelName extends RootApi<GetFreightNameResult, GetFreightNameInput> {

	/**
	 *根据labelcode获取label名称  label:标签 
	 * 
	 */
	public GetFreightNameResult Process(GetFreightNameInput inputParam, MDataMap mRequestMap) {
		...
		if(uid!=null&&!"".equals(uid)){
			...
			String sql = "select label_name from pc_labelmanage where label_code in('";
			...
			list = DbUp.upTable("pc_labelmanage").dataSqlList(sql+")", new MDataMap());
		}
		String labelName = "";
		if(!list.isEmpty()){

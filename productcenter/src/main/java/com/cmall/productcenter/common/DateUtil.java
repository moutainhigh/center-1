...
*/
	...
	* @Description:比较两个时间的大小（格式为:2015-05-10 10:10:10）
	* @param time1
	* @param time2
	* @author 张海生
	* @date 2015-5-15 上午10:32:53
	* @return  time1 大于 time2 返回正数，等于返回0，小于返回负数
	* @throws 
	*/
	public static int compareTime(String time1, String time2){
		int flag = 0;
		try {
			java.util.Date date1 = convertToDate(time1,DATE_FORMAT_DATETIME);
			java.util.Date date2  = convertToDate(time2,DATE_FORMAT_DATETIME);
	...
	/**
	 * @throws ParseException  
	* @Description:比较两个时间的大小（格式为:自定义格式）
	* @param time1
	* @param time2
	* @param timeFormat
	* @author 张海生
	* @date 2015-5-15 上午10:32:53
	* @return  time1 大于 time2 返回正数，等于返回0，小于返回负数 timeFormat 时间格式
	* @throws 
	*/
	public static int compareTime(String time1, String time2,String timeFormat){
		int flag = 0;
		try {
			java.util.Date date1 = convertToDate(time1,timeFormat);
			java.util.Date date2  = convertToDate(time2,timeFormat);
			flag = date1.compareTo(date2);
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
